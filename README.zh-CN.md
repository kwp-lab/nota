...
<p align="center">
  ...
  <img alt="版本：0.10.0" src="https://img.shields.io/badge/version-0.10.0-56615D?style=flat-square">
...
| **可选语音转写** | 仅在手动点击转写或明确开启自动转写后发送录音；可使用 Nota ASR Server、阿里云千问 DashScope 或其他 OpenAI-compatible 服务。 |
| **可复用的本地热词库** | 为人名、产品名和专业术语建立多份本地清单；每次转写选择一份并随结果保存快照，必要时可使用千问加权超级热词。 |
...
flowchart LR
    ...
    H -. "Nota ASR Server" .-> I["可续传的原始 Ogg<br>整场会议任务"]
    H -. "DashScope" .-> L["临时整场上传<br>云端异步任务"]
...
整个录音过程不需要 FFmpeg 运行时或虚拟声卡。转写是独立的可选流程。
Nota ASR Server 会把原始 Ogg 作为一个可续传、可恢复的整场会议任务处理，让
说话人标签在整场会议内统一；DashScope 使用临时整场上传和云端异步任务；其他
OpenAI-compatible Provider 继续使用临时 16 kHz 单声道 WAV 分块，分块上传后
立即删除。
...
从一级导航打开 **热词库**，可以为参会人姓名、产品名称、缩写和专业术语维护
多份可复用清单。每行填写一个热词或短语；手动开始转写时可选择“不使用”或一份
清单，也可以为自动转写单独指定清单。Nota 会给每个 transcription generation
保存不可变快照，因此之后编辑或删除清单都不会改写历史转写结果。

热词后可添加千问权重，例如 `Busabase:50` 表示超级热词。英文半角和中文全角
冒号都可以识别；不填写权重、空权重或 `0` 使用默认值。DashScope 支持
`1–5` 和 `50`；Nota ASR Server 会向支持热词的 Paraformer SeACo 与
Fun-ASR-Nano 发送纯热词文本并忽略权重。不支持热词的 Provider 会在任务创建前
明确阻止，不会静默丢弃清单后继续转写。

热词清单保存在本机 SQLite。只有本次选中的 generation 快照会随手动转写或已启用
的自动转写发送，技术日志不会记录热词正文。精确的校验、兼容性和生命周期规则见
[热词库规范](docs/hotword-library.md)。

### 可选 AI 会议文档

在 **设置 → AI 文档** 中配置 Responses API（OpenAI 官方地址只是可修改的默认值）
或 OpenAI-compatible Chat Completions 服务。完成转写后，可以在录音详情的
**AI 文档** 页签使用会议总结、
待办清单、按发言人总结和按发言人整理晨会待办四种内置模板。每次成功生成都会在
`文档\Nota\AI Documents` 下新增一个 Markdown 文件；重新生成不会覆盖旧版本，
使用外部编辑器进行的修改也会成为 Nota 预览和后续修订时使用的文档内容。
...
- 转写需要用户自行配置 Nota ASR Server、DashScope 或 OpenAI-compatible 服务
